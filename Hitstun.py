...
class Hitstun(Updateable):
    ...
    def draw(self, surface):
        ...
        start = (Config.OFFSETX, Config.OFFSETY + 100)
        end = (Config.OFFSETX + self.calc.uair5_end * Config.SCALEX, Config.OFFSETY + 100)
        pygame.draw.line(surface, Config.TRAJECTORY_COLOR, start, end, 1)

        # "hitstun" text
        rect = self.text.get_rect()
        rect.left = Config.OFFSETX
        rect.bottom = Config.OFFSETY + 100
    ...
    def draw_hit(self, surface, hit_start, hit_end, next_hit_start):
        ...
        start = (hit_start * Config.SCALEX + Config.OFFSETX, Config.OFFSETY + 100)
        end = (hit * Config.SCALEX + Config.OFFSETX, Config.OFFSETY + 100)
        pygame.draw.line(surface, color, start, end, 1)

        start = (end[0], self.calc.get_height_at_frame(hit) * Config.SCALEY + Config.OFFSETY - 10)
        end = (end[0], end[1] + 10)
        pygame.draw.line(surface, Config.TRAJECTORY_COLOR, start, end, 1)
        text = self.font.render(f"{next_hit_start - hit_end}", True, color)
        rect = text.get_rect()
        rect.top = Config.OFFSETY + 100 + 2
        rect.left = hit * Config.SCALEX + Config.OFFSETX + 5
